...
public class RoadRenderer : MonoBehaviour 
{
	...
	private Vector3[] getVerticeOffset(RoadPoint main, RoadPoint other)
	{
		...
		//create result
		result[0] = pointA + per * (0.5f * this._roadWidth);
		result[1] = pointA - per * (0.5f * this._roadWidth);
	...
	private void addSegmentQuad(RoadSegment segment)
	{
		...
		//create vertices
		Vector3 vertTL = pointA + per * (0.5f * this._roadWidth);
		Vector3 vertTR = pointA - per * (0.5f * this._roadWidth);
		Vector3 vertBL = pointB + per * (0.5f * this._roadWidth);
		Vector3 vertBR = pointB - per * (0.5f * this._roadWidth);
